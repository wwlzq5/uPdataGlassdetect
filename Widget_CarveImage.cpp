...
void Widget_CarveImage::init(int iCamNo)
{
	...
	if (pMainFrm->m_sRealCamInfo[iCameraNo].m_iImageBitCount == 24)
	{
		format = QImage::Format_RGB888;
	}
	pImageShown = new QImage(pMainFrm->m_sRealCamInfo[iCamNo].m_iImageWidth,pMainFrm->m_sRealCamInfo[iCamNo].m_iImageHeight, format);
...
int Widget_CarveImage::slots_carve()
{
	...
	pImageShown=new QImage(iImageWidth,	iImageHeight, format);// 用于实时显示
...
void Widget_CarveImage::ReturnToOriginal()
{
	...
	pImageShown = new QImage(pMainFrm->m_sRealCamInfo[iCameraNo].m_iImageWidth,pMainFrm->m_sRealCamInfo[iCameraNo].m_iImageHeight, format);
	pImageShown->setColorTable(pMainFrm->m_vcolorTable);
	pMainFrm->m_sRealCamInfo[iCameraNo].m_mutexmShownImage.unlock();
